...
def test_scrape_links():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://www.volunteermatch.org/search/?l=Canada&v=true")
            page.wait_for_timeout(10000)

            with open(output_file, "w") as file:
                while True:
                    volunteer_cards = page.locator(".pub-srp-opps__title.ga-track-to-opp-details")

                    for i in range(volunteer_cards.count()):
                        link = volunteer_cards.nth(i).get_attribute("href")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            file.write(f"https://www.volunteermatch.org{link}\n")
    ...
    old_file = "Volunteermatch/volunteermatch.txt"  # Replace with actual path
