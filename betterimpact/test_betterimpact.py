...
def test_scrape_links():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://app.betterimpact.com/PublicOrganization/c8ae5b0e-4a73-47aa-b897-67b9935a2d79/1")
            page.wait_for_timeout(3000)

            with open(output_file, "w") as file:
                
                    volunteer_cards = page.locator("//table[@class='fancy']//a")

                    for i in range(volunteer_cards.count()):
                        link = volunteer_cards.nth(i).get_attribute("href")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            file.write(f"https://app.betterimpact.com{link}\n")
    ...
    old_file = "betterimpact/betterimpact.txt"  # Replace with actual path
