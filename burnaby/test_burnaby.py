...
def test_scrape_links():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://burnaby.bcvolunteer.ca/")
            page.wait_for_timeout(2000)

            with open(output_file, "w") as file:
                while True:
                    volunteer_cards = page.locator("//div[@id='searchResultsContainer']/div/a")

                    for i in range(volunteer_cards.count()):
                        link = volunteer_cards.nth(i).get_attribute("href")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            file.write(f"https://burnaby.bcvolunteer.ca{link}\n")
    ...
    old_file = "burnaby/burnaby.txt"  # Replace with actual path
