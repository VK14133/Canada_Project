...
def test_scrape_links():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.goto("https://www.rcrg.org/VolunteerNow")
            page.wait_for_timeout(30000)

            with open(output_file, "w") as file:
                while True:
                    volunteer_cards = page.locator("ul.tableItem")

                    for i in range(volunteer_cards.count()):
                        link = volunteer_cards.nth(i).get_attribute("onclick")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            # Extract URL from: location.href='/VolunteerNow/positions/fruit-recovery-volunteer'
                            if "location.href=" in link:
                                url = link.split("location.href=")[-1].strip("'\"")
                                file.write(f"https://ehs.rosterfy.com{url}\n")
    ...
    old_file = "rcrg/rcrg.txt"  # Replace with actual path
