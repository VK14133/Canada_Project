...
def test_scrape_links():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://ehs.rosterfy.com/roles/list")
            page.wait_for_timeout(5000)

            with open(output_file, "w") as file:
                while True:
                    volunteer_cards = page.locator("//a[@class='btn btn-primary']")

                    for i in range(volunteer_cards.count()):
                        link = volunteer_cards.nth(i).get_attribute("href")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            file.write(f"https://ehs.rosterfy.com{link}\n")
    ...
    old_file = "rosterfy/ehs_rosterfy.txt"  # Replace with actual path
