...
def test_scrape_links():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://www.volunteerconnector.org/")
            page.wait_for_timeout(2000)

            with open(output_file, "w") as file:
                while True:
                    volunteer_cards = page.locator("//div[@class='tile is-parent is-6']/a")

                    for i in range(volunteer_cards.count()):
                        link = volunteer_cards.nth(i).get_attribute("href")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            file.write(f"https://www.volunteerconnector.org{link}\n")

                    next_button = page.locator("a.pagination-next")

                    if next_button.count() > 0:
                        disabled_attr = next_button.first.get_attribute("disabled")
    ...
    old_file = "volunteerconnector/volunteerconnector.txt"  # Replace with actual path
