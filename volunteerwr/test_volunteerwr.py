...
def test_scrape_links():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://search.volunteerwr.ca/Listings.php?ListType=Volunteer_Positions_Pill")
            page.wait_for_timeout(3000)

            with open(output_file, "w") as file:
                while True:
                    volunteer_cards = page.locator("//div[@class='cell2']//a")
                    '''//div[@class='cell2']/child::span[@class='value']/child::a'''
                    count = volunteer_cards.count()
                    for i in range(count):
                        link = volunteer_cards.nth(i).get_attribute("href")
                        if link and link not in seen_links:
                            seen_links.add(link)
                            file.write(f"https://search.volunteerwr.ca/{link}\n")

                    next_button = page.locator(".pgLnk.pgLnkNxt")
    ...
    old_file = "volunteerwr/volunteerwr.txt"  # Replace with actual path
